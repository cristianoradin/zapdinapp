"""
app/core/schema_baseline.py — Schema congelado (baseline) do ZapDin App.

REGRA: este arquivo NÃO cresce mais. Todo DDL aqui é idempotente
(IF NOT EXISTS / ADD COLUMN IF NOT EXISTS) e representa o estado do schema
em 2026-06-09. MUDANÇAS NOVAS vão em app/migrations/NNN_descricao.sql —
aplicadas pelo runner em app/core/migrations_runner.py.
"""
import logging
...
async def apply_baseline(conn) -> None:
    """Aplica o schema baseline completo. Idempotente — seguro re-rodar."""
    ...
    """)
    # Modos de conexão WhatsApp permitidos (CSV: servidor,local,agente)
    await conn.execute("""
        ALTER TABLE empresas ADD COLUMN IF NOT EXISTS modos_conexao TEXT DEFAULT 'servidor,local,agente'
    """)
    ...
    await conn.execute("""
        ...
        END $$;
    """)
    await conn.execute("ALTER TABLE usuarios ADD COLUMN IF NOT EXISTS email TEXT")
    ...
    for _chk_sql in [
        ...
        """DO $$ BEGIN
            IF NOT EXISTS (
                ...
                ALTER TABLE campanha_envios
                    ...
                    CHECK (status IN ('queued','paused','sent','failed','error','delivered','read'))
                    NOT VALID;
            END IF;
        END $$""",
        # Migration delivered_at/read_at em mensagens + campanha_envios
        ...
        """DO $$ BEGIN
            IF EXISTS (
                ...
                WHERE constraint_name = 'chk_envios_status' AND check_clause LIKE '%delivered%'
            ) THEN
                ALTER TABLE campanha_envios DROP CONSTRAINT chk_envios_status;
                ALTER TABLE campanha_envios
                    ADD CONSTRAINT chk_envios_status
                    CHECK (status IN ('queued','paused','sent','failed','error','delivered','read'))
                    NOT VALID;
