...
// Ícones SVG refinados (16px, stroke 1.75)
const _S = `width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"`;
const _icoEdit    = `<svg ${_S}><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>`;
const _icoTrash   = `<svg ${_S}><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg>`;
const _icoEye     = `<svg ${_S}><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>`;
const _icoCheck   = `<svg ${_S}><path d="M20 6L9 17l-5-5"/></svg>`;
const _icoRefresh = `<svg ${_S}><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg>`;
const _icoDocs    = `<svg ${_S}><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>`;
const _icoMsg     = `<svg ${_S}><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>`;

// label mapeada por title
const _btnLabels = {
  'Editar empresa': 'Editar', 'Excluir empresa': 'Excluir',
  'Aprovar': 'Aprovar', 'Reprocessar OCR': 'Reprocessar',
  'Ver documento': 'Visualizar', 'Boas-vindas': 'Boas-vindas',
};
function _btnAct(onclick, icon, title, cls='') {
  const label = _btnLabels[title] || title;
  return `<button class="btn-icon-act ${cls}" onclick="${onclick}" title="${title}">${icon} ${label}</button>`;
}
